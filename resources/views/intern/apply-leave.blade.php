{{-- C:\laragon\www\om_system\resources\views\intern\apply-leave.blade.php --}}
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
<div class="lv-layout">
    ...
    <aside class="lv-sidebar">
        <div class="lv-sb-header">
            ...
            <div class="lv-sb-role">Intern Dashboard</div>
        ...
        <div class="lv-content">
            ...
            <div class="lv-card">
                <div class="lv-card-head">
                    ...
                    <h4>Your Current Leave Balance (Entire Internship)</h4>
                </div>
                <div class="lv-card-body">
                    ...
                    <div class="lv-intern-note">
                        <i class="fas fa-info-circle"></i>
                        <strong>Note:</strong> Balance covers your entire internship period
                        ({{ \Carbon\Carbon::parse(Auth::user()->internship_start_date)->format('d M Y') }} –
                        {{ \Carbon\Carbon::parse(Auth::user()->internship_end_date)->format('d M Y') }})
                    </div>
                </div>
            </div>
                    ...
                    <form action="{{ route('leave.store') }}" method="POST" enctype="multipart/form-data" id="leaveForm">
                        ...
                        <div class="lv-form-row">
                            <div class="lv-form-group">
                                ...
                                <div class="lv-hint" id="leave_type_hint">
                                    <i class="fas fa-info-circle"></i> Interns can apply for AL, EL, Half Day (AL/EL), MC, and SL only
                                </div>
                            </div>
                            <div class="lv-form-group">
                        ...
                        <div class="lv-form-row">
                            <div class="lv-form-group">
                                ...
                                <input type="date" class="lv-input" id="start_date" name="start_date"
                                    value="{{ old('start_date') }}"
                                    max="{{ \Carbon\Carbon::parse(Auth::user()->internship_end_date)->format('Y-m-d') }}"
                                    required>
                            </div>
                            <div class="lv-form-group">
                                <label for="end_date"><i class="fas fa-calendar-day"></i> End Date <span class="lv-req">*</span></label>
                                <input type="date" class="lv-input" id="end_date" name="end_date"
                                    value="{{ old('end_date') }}"
                                    max="{{ \Carbon\Carbon::parse(Auth::user()->internship_end_date)->format('Y-m-d') }}"
                                    required>
                            </div>
                        ...
                        <div class="lv-form-row">
                            <div class="lv-form-group">
                                ...
                                <input type="text" class="lv-input" id="total_days" readonly value="0 day(s)" style="font-weight:700; color:#0EA5A0;">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // INTERN-SPECIFIC leave type config
    const leaveTypeConfig = {
        'AL': {
            name: 'Annual Leave', balance: alBalance, requiresAttachment: false,
            fixedDays: null, maxDays: null, autoCalculate: false,
            allowBackdate: false, requiresAdvance: true, advanceDays: 3,
            rules: [
                'Requires Admin approval',
                'Must be applied at least 2 days in advance',
                'Cannot be applied for today or past dates',
                'Plan accordingly.',
                'Balance will be deducted upon approval',
                `You currently have ${alBalance} days available`,
                '<strong>Note:</strong> Your balance is for entire internship period'
            ]
        },
        'HALF_DAY_AL': {
            name: 'Half Day Annual Leave', balance: alBalance, requiresAttachment: false,
            fixedDays: 0.5, maxDays: null, autoCalculate: false,
            allowBackdate: false, requiresAdvance: true, advanceDays: 3, isHalfDay: true,
            rules: [
                'Requires Admin approval',
                'Must be applied at least 2 days in advance',
                'Cannot be applied for today or past dates',
                'Plan accordingly.',
                'Counted as 0.5 days — Select AM or PM',
                'Balance will be deducted upon approval',
                `You currently have ${alBalance} days available`,
                '<strong>Note:</strong> Your balance is for entire internship period'
            ]
        ...
        'EL': {
            name: 'Emergency Leave', balance: alBalance, requiresAttachment: false,
            fixedDays: null, maxDays: null, autoCalculate: false,
            allowBackdate: true, backdateDays: 118, requiresAdvance: false,
            rules: [
                'Can be applied same day (emergency)',
                'Can be backdated up to 118 days',
                'Balance will be deducted upon approval',
                `You currently have ${alBalance} days available`,
                '<strong>Note:</strong> Your balance is for entire internship period'
            ]
        ...
        'HALF_DAY_EL': {
            name: 'Half Day Emergency Leave', balance: alBalance, requiresAttachment: false,
            fixedDays: 0.5, maxDays: null, autoCalculate: false,
            allowBackdate: true, backdateDays: 118, requiresAdvance: false, isHalfDay: true,
            rules: [
                'Can be applied same day (emergency)',
                'Can be backdated up to 118 days',
                'Counted as 0.5 days — Select AM or PM',
        ...
        'MC': {
            name: 'Medical Leave', balance: mcBalance, requiresAttachment: true,
            fixedDays: null, maxDays: null, autoCalculate: false,
            allowBackdate: true, backdateDays: 118, requiresAdvance: false,
            rules: [
                'Medical certificate (MC) is MANDATORY',
                'Can be backdated up to 118 days',
                'Balance will be deducted upon approval',
                `You currently have ${mcBalance} days available`,
                'Please submit hard copy of MC as well',
                '<strong>Note:</strong> Your balance is for entire internship period'
            ]
        },
        'SL': {
            name: 'Special Leave', balance: null, requiresAttachment: false,
            fixedDays: null, maxDays: null, autoCalculate: false,
            allowBackdate: true, backdateDays: 118, requiresAdvance: false,
            rules: [
                'For special circumstances not covered by other leave types',
                'Can be backdated up to 118 days',
                'Not deducted from leave balance',
                'Admin approval with remark required',
                'Examples: Quarantine, special personal matters, etc.',
                '<strong>Note:</strong> Leave dates must be within your internship period'
            ]
        }
    };

    const singleDayTypes = ['AL', 'EL', 'MC', 'SL'];
    ...
    leaveTypeSelect.addEventListener('change', function() {
        ...
        if (!sel) {
            ...
            leaveTypeHint.innerHTML = '<i class="fas fa-info-circle"></i> Interns can apply for AL, EL, Half Day (AL/EL), MC, and SL only';
            halfDayPeriodRow.classList.remove('show'); halfDayPeriodSelect.required = false;
            endDateInput.readOnly = false; return;
        }
        ...
        if (singleDayTypes.includes(sel) && !cfg.isHalfDay) {
            endDateInput.readOnly = true;
            if (startDateInput.value) { endDateInput.value = startDateInput.value; calculateTotalDays(); }
        } else if (!cfg.isHalfDay) { endDateInput.readOnly = false; }

        if (cfg.balance !== null) {
            availableBalanceInput.value = cfg.balance + ' days available';
            availableBalanceInput.style.color = cfg.balance > 0 ? '#0EA5A0' : '#ef4444';
        ...
        leaveTypeHint.innerHTML = '<i class="fas fa-info-circle"></i> ' + cfg.name;

        if (cfg.isHalfDay) { startDateInput.value = ''; endDateInput.value = ''; totalDaysInput.value = '0 day(s)'; }
    ...
    startDateInput.addEventListener('change', function() {
        ...
        if (cfg.isHalfDay && sd) {
            endDateInput.value = sd; totalDaysInput.value = '0.5 day (Half Day)'; totalDaysInput.style.color = '#0EA5A0';
    ...
    function calculateTotalDays() {
        ...
        if (cfg && cfg.balance !== null && days > cfg.balance) {
            totalDaysInput.style.color = '#ef4444';
            alert('Warning: You only have ' + cfg.balance + ' days available. Your application may be rejected.');
        }
    }
